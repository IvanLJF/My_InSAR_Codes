...
20000508
20000717
20000925];    % master name, directories, 
                      % remove a bad image taken on 19960114
...
for i=1:length(dib)
    for j=i:length(dib1)
        if B1(i,j)~=-10000
            ...
            SB(k)=B1(i,j);
            if pair_dates(k,3)>=1464    % remove the interferogram with temporal baseline equal and greater than 1464 days
                k=k-1;
            end
        end
    end
...
% save interferogram information
fid = fopen('E:\PhoenixSAR\Dif_Int_MLI\86ints.all', 'w');
fprintf(fid,'%d  %d  %d  %d\n', k, 3200, 3500, 13920);
fprintf(fid,'%s\n', 'E:\PhoenixSAR\Dif_Int_MLI\');
for i=1:k
    str=[num2str(pair_dates(i,1)), '_', num2str(pair_dates(i,2)), '.diff.int2.ph'];
    fprintf(fid,'%s\n', str);
end
fclose(fid);
...
%      xT=[datenum(num2str(pair_dates(i,1)), 'yyyymmdd'), datenum(num2str(pair_dates(i,2)), 'yyyymmdd')];
%      yN=[i, i];
%      zB=[pair_dates(i,4), pair_dates(i,4)];
%      %plot(xT, yN, 'LineWidth', 1.5, 'Color', 'k');
%      plot3(zB, yN, xT, 'LineWidth', 1.5, 'Color', 'b'); %[0 0 0]);
%      hold on;
% end;
% hold off;
% datetick('z', 'yyyy');
% set(gca, 'FontSize', 10.5);
% set(gca, 'TickLength', [0.012, 0.012]);
% zlabel('Year');
