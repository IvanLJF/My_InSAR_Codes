function [x, VV, delta]=lsupdate(num_PS, num_Arcs, starting, ending, PSCoor, Arcs, dvhc)
% 通过二次最小二乘预测每个点的绝对位移矢量和高程误差，并剔除矢量中的粗差以及高程误差增量
%   [x, VV, delta]=lsupdate(num_PS, num_Arcs, start, PSCoor, Arcs, dvhc);
%
...
%  e.g.,   num_PS=50;
%            num_Arcs=133;
%            [x, VV, delta]=lsupdate(num_PS, num_Arcs, starting, ending, PSCoor, Arcs, dvhc);
% 
...
[A, L, x, VV, delta]=grserrls_ne(num_PS, num_Arcs, starting, ending, 0.45, Arcs, dvhc(:,1), dvhc(:,3));   % for velocity
clear A L;
