...
FUNCTION TLI_RESULTFILE, master, slave, mlook_a, mlook_r,i_outfile=i_outfile,p_outfile=p_outfile, $
                    c_outfile= c_outfile, master_ss=master_ss, master_ls= master_ls
  ...
  IF ~KEYWORD_SET(mlook_a) OR ~KEYWORD_SET(mlook_l) THEN BEGIN
...
;    mlook_r=3
  ...
  PRINT, k,'  /',tile_l
    ...
    start_y= k*tile_size
    masterSLC= SUBSETSLC(master, 0, master_ss, start_y, end_y-start_y+1)
    slaveSLC= SUBSETSLC(slave, 0, master_ss, start_y, end_y-start_y+1)
