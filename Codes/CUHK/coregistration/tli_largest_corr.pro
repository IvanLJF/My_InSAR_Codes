...
FUNCTION TLI_LARGEST_CORR, master_file, slave_file, master_ss, master_ls, slave_ss, slave_ls, $
...
;-----------------------------------------------------------------  
  ...
  subset_end_s= LONG64(master_s+FLOOR(winsub_or/2))
  subset_start_l= LONG64(master_l-FLOOR(winsub_oazi/2))
  subset_end_l= LONG64(master_l+FLOOR(winsub_oazi/2))
  search_start_s= LONG64(master_s+s_offset-FLOOR(winsearch_or/2))
  search_end_s= LONG64(master_s+s_offset+FLOOR(winsearch_or/2))
  search_start_l= LONG64(master_l+l_offset-FLOOR(winsearch_oazi/2))
  search_end_l= LONG64(master_l+l_offset+FLOOR(winsearch_oazi/2))
  IF KEYWORD_SET(master_swap_endian) THEN BEGIN
    master_subset= TLI_SUBSETDATA(master_file, master_ss, master_ls, $
                                  subset_start_s, subset_end_s-subset_start_s, subset_start_l, subset_end_l-subset_start_l, $
                                  /float,/swap_endian)
  ENDIF ELSE BEGIN
    master_subset= TLI_SUBSETDATA(master_file, master_ss, master_ls, $
                                  subset_start_s, subset_end_s-subset_start_s, subset_start_l, subset_end_l-subset_start_l, $
  ...
  IF KEYWORD_SET(slave_swap_endian) THEN BEGIN  
    slave_search= TLI_SUBSETDATA(slave_file, slave_ss, slave_ls, $
                                  search_start_s, search_end_s-search_start_s, search_start_l, search_end_l-search_start_l, $
                                  /float,/swap_endian)
  ENDIF ELSE BEGIN
    slave_search= TLI_SUBSETDATA(slave_file, slave_ss, slave_ls, $
                                  search_start_s, search_end_s-search_start_s, search_start_l, search_end_l-search_start_l, $
