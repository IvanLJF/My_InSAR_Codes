...
PRO TLI_RG_DVDDH_NOWEIGHT,plistfile, dvddhfile, vdhfile, ptattrfile, mask_arc, mask_pt_coh, refind,v_acc, dh_acc
  COMPILE_OPT idl2
    ...
    FOR i=0, N_ELEMENTS(startpts)-1 DO BEGIN ; Parent node
      ...
      FOR j=0, N_ELEMENTS(startpt_con)-1 DO BEGIN ; Child node
        ...
        IF startpt_arc[0] EQ startpt THEN BEGIN ; start point of the arc is equal to startpt
          ...
          pt_attr[endpt].v= pt_attr[startpt].v $
            + ref_v $
            + startpt_arc_info[2] ; Update end node's attr. dv= small_ind-large_ind
          pt_attr[endpt].dh= pt_attr[startpt].dh $
            + ref_dh $
            + startpt_arc_info[3]  ; Update end node's attr. dv= small_ind-large_ind
