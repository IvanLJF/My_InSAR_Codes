...
; ---- ESTIMATION POLYNOMIALE DE ALPHA ----
	...
	u_rg = total((smooth(u,3,/ed))*mask_cross,2)/(total(mask_cross,2)+eps)
	u_fit = complexarr(file.xdim)+mean(u_rg)

	v_rg = total((smooth(v,3,/ed))*mask_cross,2)/(total(mask_cross,2)+eps)
	v_fit = complexarr(file.xdim)+mean(v_rg)

	w_rg = total((smooth(w,3,/ed))*mask_cross,2)/(total(mask_cross,2)+eps)
	w_fit = complexarr(file.xdim)+mean(w_rg)

	z_rg = total((smooth(z,3,/ed))*mask_cross,2)/(total(mask_cross,2)+eps)
