...
; calculating span
	...
	for i=0,anz_blocks-1 do begin
		...
		if file.type eq 601 then begin
			oblock = abs(smooth(block[channel1 mod file.vdim,channel1 / file.vdim,*,*] * conj(block[channel2 mod file.vdim,channel2 / file.vdim,*,*]),smm,/ed) / $
			sqrt(smooth(abs(block[channel1 mod file.vdim,channel1 / file.vdim,*,*])^2,smm,/ed)*smooth(abs(block[channel2 mod file.vdim,channel2 / file.vdim,*,*])^2,smm,/ed)))

		endif else begin
			if matrixflag eq 0 then oblock = abs(smooth(block[0,channel1,*,*] * conj(block[0,channel2,*,*]),smm,/ed))/sqrt(smooth(abs(block[0,channel1,*,*])^2,smm,/ed)*smooth(abs(block[0,channel2,*,*])^2,smm,/ed))
