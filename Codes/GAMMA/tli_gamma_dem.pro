...
FUNCTION TLI_DEMNAMES, range, suffix=suffix
  ...
  ENDIF ELSE BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      lonanns= lonsann+STRCOMPRESS(CEIL(lone)+LINDGEN(CEIL(lons)-CEIL(lone)+1),/REMOVE_ALL)
    ENDELSE
  ENDELSE
  ...
  FOR i=0, lon_ann_no-1 DO BEGIN
    FOR j=0, lat_ann_no-1 DO BEGIN
      IF STRLEN(lonanns[i]) EQ 3 THEN BEGIN
        names[k]= latanns[j]+STRMID(lonanns[i],0,1)+STRCOMPRESS(0,/REMOVE_ALL)+STRMID(lonanns[i], 1, 2)
      ENDIF ELSE BEGIN
...
PRO TLI_GAMMA_DEM
  ...
  sourcepath='/mnt/data_tli/Data/DEM/SRTM/'
  workpath='/mnt/data_tli/Data/DEM/Yyueyang_CSM'
  range= '112.8E 113.4E 29.1N 29.7N' ; Startx endx starty endy
  name='Yueyang'    ; Annotation of DEM files.
  ...
  IF NOT FILE_TEST(workpath,/DIRECTORY) THEN FILE_MKDIR, workpath
  
  ; Check file existence.
  FOR i=0, ndems-1 DO BEGIN
    dems_i=FILE_SEARCH(sourcepath, names[i])
    IF NOT FILE_TEST(dems_i) THEN Message, 'Error! File does not exist:'+sourcepath+names[i]
    Print, 'File:'+STRCOMPRESS(i+1)+'/'+STRCOMPRESS(ndems)+' was found:'+sourcepath+names[i]
    dems[i]=dems_i
  ENDFOR
  
  ; Copy files.
  Print, 'Copying files...'
  FILE_COPY, dems, workpath,/verbose,/OVERWRITE
  ...
  FOR i=0, ndems-1 DO BEGIN
    ...
    PrintF, lun, ''
  ENDFOR
  PrintF, lun, 'rm -f temp_dem temp_dem2 '+name+'.dem '+name+'.dem.par'
