PRO CWN_SMC_CREATEDIFFPAR_EVENT, EVENT
  ...
  END
  'openslave':begin
  temp=WIDGET_INFO((*pstate).openslave,/droplist_select)
  IF temp EQ 0 THEN BEGIN
    WIDGET_CONTROL, (*pstate).slave,set_value='-', set_uvalue='-'
    WIDGET_CONTROL, (*pstate).slave, sensitive=0
    RETURN
  ENDIF
  
  
  WIDGET_CONTROL, (*pstate).slave, sensitive=1
  slave=config.s_slc
  infile=dialog_pickfile(title='Sasmac InSAR',/read,filter='*.pwr', path=workpath,file=slave)
  IF NOT FILE_TEST(infile) THEN return
...
ENDIF ELSE BEGIN
  ...
  IF NOT FILE_TEST(slave) THEN BEGIN
    void=DIALOG_MESSAGE('Error! Please select input slave file.',/center,/information)
    Return
  ENDIF
  WIDGET_CONTROL, (*pstate).slave, get_value=slave
ENDELSE
...
Case algorithm OF
  ...
  1: algorithm='0'
  2: algorithm='2'
  Else: Message, 'Error! Method not supported!'
ENDCASE
...
PRO CWN_SMC_CREATEDIFFPAR
  ...
  IF FILE_TEST(m_slc) THEN BEGIN
    ...
    IF suffix EQ '.pwr' THEN BEGIN
      parfile=m_slc+'.par'
      finfo=TLI_LOAD_SLC_PAR(parfile)
      width=STRCOMPRESS(finfo.range_samples,/REMOVE_ALL)
    ENDIF
    outputfile=workpath+TLI_FNAME(m_slc, /nosuffix)+'.diff_par'
    slcinfo=TLI_LOAD_PAR(m_slc+'.par',/keeptxt)
    width=slcinfo.range_samples
  ENDIF
  ...
  openmaster=widget_button(mastesID,value='Master PWR',uname='openmaster',xsize=90)
  
  slaveID=widget_base(tlb,/row,xsize=xsize,frame=1,/align_left)
  slave=widget_text(slaveID,value='',uvalue='',uname='slave',/editable,xsize=68,sensitive=0)
  openslave=widget_droplist(slaveID,value=['Ignore Slave','Select Slave'],xsize=90,uname='openslave')
  ...
  algorithm=widget_droplist(temp, value=[$
    'SLC/MLI_par ISP SLC/MLI parameter files',$
    'OFF_par Interferogram/offset parameter files',$
