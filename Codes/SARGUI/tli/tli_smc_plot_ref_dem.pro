...
; History
;
PRO TLI_SMC_PLOT_REF_DEM_EVENT, EVENT
  ...
  Case uname OF
    'openinput':BEGIN
    inputfile=dialog_pickfile(title='Sasmac InSAR',/read,filter='*dem_seg*', path=workpath,/must_exist)
  ...
  'opendemseg': BEGIN
    inputfile=dialog_pickfile(title='Sasmac InSAR',filter="*dem_seg*",/read, path=workpath)
...
IF 1 THEN BEGIN
  ...
  TLI_SMC_DISPLAY, fig
ENDIF ELSE BEGIN
  result=dialog_message('Plot interferometric DEM: '+STRING(10b)+'Finished successfully.'+STRING(10b)+'Quit?',title='Plot interferometric DEM',/question,/center)
ENDELSE
...
PRO TLI_SMC_PLOT_REF_DEM
  ...
  refdem=workpath+'dem_seg'
  dem_seg=workpath+'dem_seg'
  IF NOT FILE_TEST(refdem) THEN BEGIN
    refdem=''
    img=''
  ENDIF ELSE BEGIN
    img=refdem+'.jpg'
  ...
  tlb=widget_base(title='Plot Interferometric DEM',tlb_frame_attr=0,column=1,xsize=xsize,ysize=ysize,xoffset=xoffset,yoffset=yoffset)
  
  inID=widget_base(tlb,row=1,xsize=xsize,frame=1)
  input=widget_text(inID,value=refdem,uvalue=refdem,uname='input',/editable,xsize=73)
  ...
  output=widget_text(outID,value=img,uvalue='ouput',uname='output',/editable,xsize=63)
  ...
  xmanager,'TLI_SMC_PLOT_REF_DEM',tlb,/no_block
