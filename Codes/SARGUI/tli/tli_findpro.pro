...
PRO TLI_FINDPRO, proname
  ...
  Case nfiles OF
    ...
    0: BEGIN
      info=['The function is not found in the given path:',proname]
    END
    
    1: BEGIN
      info=['This is a function without GUI.', $
            '', $
            '', $
           'The function is in the following path:',$
           prolist]
    END
    ...
    ELSE: BEGIN
      info=['This is a function without GUI.', $
            '', $
            '', $
           'The functions are in the following path:',$
           STRJOIN(prolist)]
    END
    
  ENDCASE
  
  void=DIALOG_MESSAGE(info,/info,/center, TITLE='R&D Dept, Sasmac - For developers only.')
