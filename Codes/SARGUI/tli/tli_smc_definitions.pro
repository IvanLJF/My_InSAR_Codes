...
PRO TLI_SMC_DEFINITIONS,update_pref=update_pref
  ...
  if config.os eq 'windows' then begin
    TLI_SMC_DUMMY, inputstr=['This is a non-Unix OS.',$
                             'Enjoy the errors might be encountered.',$
                             'But MOST of the functions will work fine.'],/info
  ...
  config.temppath  = config.tempbase+'TMPSMC'+path_sep()
  IF NOT FILE_TEST(config.temppath,/DIRECTORY) THEN file_mkdir,config.temppath,/noexpand_path
