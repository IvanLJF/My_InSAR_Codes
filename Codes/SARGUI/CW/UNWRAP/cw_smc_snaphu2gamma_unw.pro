@ tli_format_convert
  pro cw_smc_snaphu2gamma_unw_event,EVENT
  ...
  case uname of
    ...
    'data_in_button': begin
      ...
      infile=dialog_pickfile(title='open data ',filter='*.snaphu_ini',/read,/must_exist, path=config.workpath)
    ...
    lines=STRCOMPRESS(finfo.azimuth_lines-1,/REMOVE_ALL)
      ...
      workpath=config.workpath
      data_in=workpath+TLI_FNAME(inputfile, /remove_all_suffix)+'.snaphu_ini'
       data_out=workpath+TLI_FNAME(inputfile, /remove_all_suffix)+'.flt.filt.unw'
     endif
   ...
   IF NOT FILE_TEST(data_out) THEN begin
    ...
    if width le 0 then begin
      ...
      fpath=strsplit(data_in,'\',/extract);
      pathsize=size(fpath)
      ...
      temp=hdrpath+file(0)+'.snaphu_ini.convert'
      
    width=strcompress(long(width),/remove_all)
   
    ;  scr="swap_bytes "+data_in+" "+data_out+" 4"
   ;    spawn,scr
   tli_format_convert,data_in,width,output_format='float','alt_line_data',/output_swap_endian
  scr="mv "+temp+" "+data_out
  tli_smc_spawn, scr ,info='Use Snaphu to Unwarp, Please wait...'
scr="rasrmg "+data_out+" - "+width
 tli_smc_spawn, scr ,info='Ras Unwarp Image, Please wait...'
...
END
PRO cw_smc_snaphu2gamma_unw,EVENT
  ...
  tlb=widget_base(title='SASMAC_SNAPHU2GAMMA',tlb_frame_attr=0,/column,xsize=xsize,ysize=ysize,xoffset=xoffset,yoffset=yoffset)
 ...
 ; inID=widget_base(tlb,/row,xsize=xsize,frame=1)
  ...
  xmanager,'cw_smc_snaphu2gamma_unw',tlb,/no_block
END