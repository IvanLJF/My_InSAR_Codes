  pro cw_smc_gamma2snaphu_cc_event,EVENT
    ...
    case uname of
      ...
      'data_in_button': begin
        infile=dialog_pickfile(title='open data ',filter='*.filt.cc',/read,/must_exist, path=config.workpath)
        ...
        IF FILE_TEST(config.inputfile) THEN begin
          ...
          workpath=config.workpath
          data_in=workpath+TLI_FNAME(inputfile, /remove_all_suffix)+'.filt.cc'
          data_out=workpath+TLI_FNAME(inputfile, /remove_all_suffix)+'.filt.cc.convert'
        endif
        ...
        widget_control, (*pstate).parlabel, set_value=parlab, set_uvalue=parfile
      end
      ...
      'data_out_button': begin
        ...
        IF NOT FILE_TEST(data_in) THEN begin
          TLI_SMC_DUMMY, inputstr=['Error!', 'Please select The correct Data_In']
          return
        endif
        ...
        file=data_out+'.filt.cc.convert'
        
        infile=dialog_pickfile(title='output data',filter='*.filt.cc.convert',file=file,/write,/overwrite_prompt, path=config.workpath)
      ...
      IF data_out EQ '' THEN begin
        TLI_SMC_DUMMY, inputstr=['Error!', 'Please select The Correct Data_Out']
        return
      endif
      ...
      temp=config.workpath+TLI_FNAME(data_in, /remove_all_suffix)+'.filt.cc.convert'
      
      TLI_SMC_PROGRESS, message='Converting file format: GAMMA -> Snaphu'
      ...
      if temp ne data_out then begin
        scr="mv "+temp+" "+data_out
        tli_smc_spawn, scr ,info='Converting CC_File Gamma To Snaphu, Please wait...'
      endif
    end
...
END
PRO cw_smc_gamma2snaphu_cc,EVENT
  ...
  xmanager,'cw_smc_gamma2snaphu_cc',tlb,/no_block
END