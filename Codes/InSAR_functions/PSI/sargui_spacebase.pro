...
PRO SARGUI_SPACEBASE_EVENT,EVENT
...
case uname of
  ...
  'ok':begin
    ...
    result=dialog_message('嘿嘿，老桑正画图呢',/information,title='等会再点')
    end
  'del':begin
    widget_control,(*pstate).list,get_uvalue=names
...
;    if list_num lt 2 then result=dialog_message('请选择至少两幅影像',title='影像数目过少',/information)
    ...
    if min(list_select)eq 0 then names=names(max(list_select):list_num-1)
  ...
  'input':begin
    widget_control,(*pstate).list,get_uvalue=names
    infiles=dialog_pickfile(title='请选择影像列表文件',/read)
  ...
  'output':begin
    widget_control,(*pstate).list,get_uvalue=names
    outfile=dialog_pickfile(title='请选择输出路径',/write)
...
PRO SARGUI_SPACEBASE,EVENT
;-创建组件
device,get_screen_size=screen_size
tlb=widget_base(row=1,title='计算空间基线',tlb_frame_attr=1,xsize=330,xoffset=screen_size(0)/3,yoffset=screen_size(1)/3)
;-列表组件
...
okID=widget_button(fun,value='确定',uname='ok')
clID=widget_button(fun,value='取消',uname='cl')
;-创建结构体
state={list:list}
pstate=ptr_new(state)
widget_control,tlb,set_uvalue=pstate
widget_control,tlb,/realize
xmanager,'SARGUI_SPACEBASE',tlb,/no_block
END