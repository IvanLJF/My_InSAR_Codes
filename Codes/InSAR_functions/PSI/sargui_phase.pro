...
case uname of 
  ...
  'openoutput':begin
    ...
    file=file+'.phase.bmp'
    outfile=dialog_pickfile(title='选择输出文件',/write,filter='*.bmp',file=file,/overwrite_prompt)
    if outfile eq '' then return
;    ;-检查输出文件是否是bmp扩展名
;    outpath=strsplit(outfile,'\',/extract)
;    pathsize=size(outpath)
;    outname=outpath(pathsize(1)-1)
;    file=strsplit(outname,'.',/extract)
;    filesize=size(file)
;    if filesize(1)eq 1 then outfile=outfile+'.bmp';如果文件名没有后缀，自动添加后缀
;    ;-检查文件名是否重复，重复则进行提示
;    files=findfile(outfile,count=numfiles)
;    if numfiles eq 1 then begin
;      result=dialog_message('文件名已经存在，是否覆盖',/question,/default_no)    
...
;    endelse
  ...
  'ok':begin
    ...
    if infile eq '' then result=dialog_message('请选择输入文件',title='选择输入文件',/information)
    if columns le 0 then result=dialog_message('文件行数为正',title='行数错误',/information)
    if lines le 0 then reslut=dialog_message('文件列数为正',title='列数错误',/information)
    if output eq '' then result=dialog_message('请选择输出文件',title='选择输出文件',/information)
    slc=openslc(infile)
    rl_part=real_part(slc)
    img_part=imaginary(slc)
;    slcamplitude=sqrt(rl_part^2+img_part^2)
    phase=atan(img_part/rl_part)
    write_bmp,output,phase
...
PRO SARGUI_PHASE,EVENT
;-创建组件
device,get_screen_size=screen_size
xoffset=screen_size(0)/3
yoffset=screen_size(1)/3
tlb=widget_base(title='SLC转化为相位影像',tlb_frame_attr=1,column=1,xsize=260,ysize=150,xoffset=xoffset,yoffset=yoffset)
;-创建输入文件组件
...
xmanager,'SARGUI_PHASE',tlb,/no_block
END