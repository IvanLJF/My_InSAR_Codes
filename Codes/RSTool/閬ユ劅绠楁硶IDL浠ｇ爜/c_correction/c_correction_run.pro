...
if (map_info.proj.name ne map_info_dem.proj.name) or (map_info.rotation ne map_info_dem.rotation) then $
...
topo_doit, bptr=[0,1], dims=dims1, pos=0, kernel=kernel, $
...
;   window,10+k,xsize=500,ysize=400,xpos=100+10*k,ypos=100+10*k, $
;     title="Regression on Band "+strtrim(pos_iage[k]+1,2)
...
;   plot,cos_gamma_i[mask],band[mask],psym=3,xrange=[0,1],color=0,background='FFFFFF'XL
;   oplot,[0,1],[b,m+b],psym=-1,color=0
