...
PRO gpukkmeans_run_define_buttons, buttonInfo
   ENVI_DEFINE_MENU_BUTTON, buttonInfo, $
      VALUE = 'Kernel K-means (CUDA)', $
      REF_VALUE = 'K-Means', $
      EVENT_PRO = 'gpukkmeans_run', $
      UVALUE = 'GPUKKMEANS',$
      POSITION = 'after'
END
...
while change and (iter lt 100) do begin
   progressbar->Update,iter, $
      text='Training: iter='+strtrim(iter,2) 
   change = 0
   U = fltarr(m,K)
   for i=0,m-1 do U[i,labels[i]] = 1
   MU = diag_matrix(1/(total(U,1)+1))##U
; Z is a K by m array 
...
; loop over rows
   ...
   while i lt num_rows do begin
      pct=i*100/num_rows
      progressbar->Update,fix(pct),text='Classifying: '+strtrim(pct,2)+'%'
