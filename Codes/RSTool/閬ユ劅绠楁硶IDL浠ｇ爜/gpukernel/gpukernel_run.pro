...
PRO gpukernel_run_define_buttons, buttonInfo
   ENVI_DEFINE_MENU_BUTTON, buttonInfo, $
      VALUE = 'Gaussian Kernel (CUDA)', $
      REF_VALUE = 'Support Vector Machine', $
      EVENT_PRO = 'gpukernel_run', $
      UVALUE = 'KERNEL',$
      POSITION = 'after'    
END
...
COMPILE_OPT STRICTARR
   ...
   if cuda then begin
      if symm then Kappa_gpu = gpugausskernel_matrix(Gs_gpu,gma=0.5/sigma^2) $
      else begin
         Hs_gpu = gpuputarr(Hs)
         Kappa_gpu = gpugausskernel_matrix(Gs_gpu,Hs_gpu,gma=0.5/sigma^2)
         gpufree,Hs_gpu
      endelse      
...
;    too many training data so sample uniformly (with replacement)
...
; bracket the minimum
   ...
   plot,[alog10(s1),alog10(s3)],[theta(s1),theta(s3)],$
      ...
      title = 'Gaussian Kernel Classification',$
      xtitle = 'log(sigma)', $
      ytitle = 'theta'
